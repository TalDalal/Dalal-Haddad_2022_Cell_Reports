function [STA,coherenceCurve,STA_shuff_mean,STA_shuff_SD,coherenceCurve_shuff_mean,coherenceCurve_shuff_SD,freqs, spkTimesSam] = getCoherence(spkTimes,inputSignal,Fs,windowSize,iterations,plotFlag,varargin)
...
% get the STA and coherence curve
    function [STA,coherenceCurve,freqs] = getCohCurve(spkTimesSam,windowSizeSam,Fs,inputSignal)
        ...
        if size(stimSegments,1)==1% not enough spikes for STA
            warning('not enough spikes for STA')
            STA = nan(size(stimSegments));
            coherenceCurve = nan(1,lengthFFT);
            freqs = Fs*(0:(lengthSeg/2))/lengthSeg;
        else
